 ...
 * Supports two contexts: "calendar" (calendar-specific prompt) and "general" (neutral prompt).
...
export type TranscribeContext = "calendar" | "general";
...
export async function transcribeVoice(filePath: string, context: TranscribeContext = "calendar"): Promise<string> {
  ...
  try {
    ...
    const prompt = context === "general" ? TRANSCRIBE_PROMPT_GENERAL : TRANSCRIBE_PROMPT_CALENDAR;
