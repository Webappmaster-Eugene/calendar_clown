...
3. Оцени вес каждого продукта в граммах, ориентируясь на:
   ...
   - ПЛОТНОСТЬ ПРОДУКТОВ: лёгкие объёмные продукты весят значительно меньше, чем кажется визуально:
     * Листовой салат, руккола, шпинат: полная миска ≈ 30-50г (НЕ 70-100г)
     * Черри-томаты: 1 штука ≈ 8-10г
     * Свежая зелень (укроп, петрушка, кинза): горсть ≈ 5-10г
     * Тонкие ломтики огурца: выглядят объёмнее, чем весят
     * Ягоды (клубника, малина): горсть ≈ 30-50г
     * Общее правило: для лёгких и объёмных продуктов округляй вес ВНИЗ
...
ВАЖНО: В оценке упоминай ТОЛЬКО те продукты, которые ты перечислил в items. Не додумывай и не упоминай продукты, которых нет в списке items.

Поле "dish_type" — краткое описательное название блюда/набора блюд (например: "Яичница с овощами и тостом", "Борщ с хлебом", "Салат Цезарь"). НЕ пиши "Обед"/"Завтрак"/"Ужин".
...
Ответь СТРОГО в формате JSON без markdown-обёртки, без текста до и после JSON:
{
  ...
  "dish_type": "Куриная грудка гриль с рисом и овощами",
  "meal_assessment": "Сбалансированный приём пищи с хорошим содержанием белка...",
  "confidence": "high"
}
...
export function buildCatalogBlock(catalog: AnalyzeFoodOptions): string {
  ...
  catalog.products.forEach((p, i) => {
    ...
    lines.push(
      `${i + 1}. [id=${p.id}] "${p.name}" (${p.unit}) — ${cals} ккал, Б ${prot}, Ж ${fats}, У ${carbs}${descSuffix}`,
    );
  });

  if (catalog.total > catalog.products.length) {
    lines.push(`…(показано ${catalog.products.length} из ${catalog.total})`);
  }

  lines.push("");
  lines.push(
    'ЕСЛИ ты визуально идентифицируешь один из этих продуктов на блюде — используй ТОЧНО эти значения ' +
      '(масштабируя от 100 г/100 мл на оценённый вес порции). В поле "name" запиши название из каталога как есть, ' +
      'а в поле "matched_product_id" укажи id из каталога (число из [id=...]). ' +
      'Если уверенности в совпадении нет — НЕ ставь matched_product_id и рассчитывай продукт стандартным способом.',
  );
...
function validateNutritionResult(data: Record<string, unknown>): NutritionResult {
  ...
  const validatedItems: FoodItem[] = items.map((item: Record<string, unknown>) => {
    const rawMatched = item.matched_product_id;
    const matchedProductId =
      typeof rawMatched === "number" && Number.isInteger(rawMatched) && rawMatched > 0
        ? rawMatched
        : undefined;
    return {
      ...
      ...(matchedProductId !== undefined ? { matched_product_id: matchedProductId } : {}),
    };
  });
